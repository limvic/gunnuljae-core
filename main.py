...
async def index_snapshot():
    """
    실시간 지수 스냅샷
    - KOSPI / KOSDAQ : Naver 금융 크롤링
    - NASDAQ / S&P500 : Yahoo Finance API
    캐시 TTL: 60초
    """
    from bs4 import BeautifulSoup
    ...
    HEADERS = {"User-Agent": "Mozilla/5.0 (Linux; Windows NT 10.0) AppleWebKit/537.36"}
    ...
    # ── 1. KOSPI / KOSDAQ (Naver 금융)
    try:
        async with httpx.AsyncClient(timeout=8, headers=HEADERS) as c:
            res = await c.get("https://finance.naver.com/")
        soup = BeautifulSoup(res.text, "html.parser")

        def parse_naver_index(id_prefix):
            try:
                val_el = soup.select_one(f"#{id_prefix}_value") or soup.select_one(f"[id$='{id_prefix}_value']")
                chg_el = soup.select_one(f"#{id_prefix}_change") or soup.select_one(f"[id$='{id_prefix}_change']")
                rat_el = soup.select_one(f"#{id_prefix}_rate")   or soup.select_one(f"[id$='{id_prefix}_rate']")
                if not val_el:
                    return None
                price  = float(val_el.text.strip().replace(",", ""))
                change = float(chg_el.text.strip().replace(",", "").replace("+","")) if chg_el else None
                c_pct  = float(rat_el.text.strip().replace(",", "").replace("+","").replace("%","")) if rat_el else None
                return {"price": price, "change": change, "change_pct": c_pct}
            except Exception:
                return None

        result["kospi"]  = parse_naver_index("KOSPI")
        result["kosdaq"] = parse_naver_index("KOSDAQ")
    except Exception as e:
        pass  # 실패 시 None 유지 → 프론트에서 "--" 표시

    # ── 2. NASDAQ / S&P500 (Yahoo Finance crumb-free quote API)
    try:
        async with httpx.AsyncClient(timeout=8, headers={
            **HEADERS,
            ...
            res = await c.get(
                ...
                params={"symbols": "^IXIC,^GSPC", "range": "1d", "interval": "1d"}
            )
        data = res.json()
        items = data.get("spark", {}).get("result") or []
        for item in items:
            sym    = item.get("symbol", "")
            ...
            entry = {"price": round(price, 2), "change": change, "change_pct": change_pct}
            if sym == "^IXIC":
                result["nasdaq"] = entry
            elif sym == "^GSPC":
                result["sp500"]  = entry
