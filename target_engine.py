...
async def get_intraday_vwap(code: str) -> dict:
    ...
    price        = _to_int(o.get("stck_prpr"))      # 현재가
    acml_vol     = _to_int(o.get("acml_vol"))        # 누적 거래량 (주)
    acml_pbmn    = _to_int(o.get("acml_tr_pbmn"))    # 누적 거래대금 (원)

    # 실측 누적값 없으면 진입 금지 (추정 금지)
    if acml_vol <= 0 or acml_pbmn <= 0 or price <= 0:
        log.info(f"[vwap] 누적 데이터 없음 ({code}): vol={acml_vol} pbmn={acml_pbmn}")
        return fail

    vwap = round(acml_pbmn / acml_vol)
    if vwap <= 0:
        return fail

    vwap_gap = round((price - vwap) / vwap * 100, 2)
    result = {
        "vwap": vwap,
