...
async def fetch_price(code: str, token: str) -> dict:
    ...
    return {
        ...
        # 🔍 임시 디버그: KIS output에서 거래대금/종목명 후보 키 노출
        "_debug_keys": [k for k in o.keys() if "pbmn" in k or "tr_" in k or "isnm" in k],
    }
...
async def get_stock(code: str):
    ...
    try:
        ...
        # 당일 누적거래대금/누적거래량 실측. 실패 시 vwap=None (추정 금지).
        from target_engine import get_intraday_vwap
        vwap_data = await get_intraday_vwap(code)
        result = {**p, **pat, **vwap_data, "code": code, "status": "ok"}
