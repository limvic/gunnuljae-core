...
async def wave_alert_loop():
    """
    🌊 Wave Alert 백그라운드 루프 (별도 인스턴스 X — 기존 패턴 재사용).
    - 장 중: 5분(300초) 간격 실행
    - 장 외: 60초 sleep 후 재확인
    """
    print("[WAVE-ALERT] 🌊 스케줄러 시작 — v1.0")
    ...
    while True:
        try:
            if _scheduler_market_open():
                ...
                now_str = datetime.now(KST).strftime("%H:%M")
                print(f"[WAVE-ALERT] 💤 장외 대기 ({now_str}) — 60초 후 재확인")
                await asyncio.sleep(60)
        except Exception as e:
