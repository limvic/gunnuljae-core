...
JUDGE_STOCK_MAP = {
    ...
    "HD현대일렉트릭": "267260",   # ✅ 검증완료: 267260(전기장비). 267250은 HD현대 지주
    "HD현대": "267250",           # 지주 — 혼동 방지용 별도 등록
    "이수페타시스": "007660", "LG디스플레이": "034220",
    # … 관심종목 계속 확장 (코드는 반드시 검증 후 추가)
}
...
async def judge_step1(query: str):
    ...
    return {
        "code": code,
        # ✅ 정정: hts_kor_isnm(종목명) 우선 → bstp_kor_isnm은 업종명이라 fallback
        "name": o.get("hts_kor_isnm") or o.get("bstp_kor_isnm") or code,
        "sector": o.get("bstp_kor_isnm", ""),   # 업종은 별도 필드로 분리
        "price":  price,
