...
SUPPORT_CLUSTER_TOL = 0.008   # 군집 묶음 허용폭 ±0.8%
...
def _sup_cluster(levels, price):
    """지지 후보를 가격 가까운 것끼리 군집화. 현재가 이하만 = 진짜 지지(위는 저항).
    반환: 군집 리스트(현재가에 가까운 순). 각 군집:
      {price(대표=군집 최저, 보수적), members[], count, has_vol_node}."""
    ...
    for name, val in cand:
        placed = False
        for cl in clusters:
            if abs(val - cl["_ref"]) / cl["_ref"] <= SUPPORT_CLUSTER_TOL:
...
async def support_zone(query: str, mode: str = "balance"):
    ...
    clusters = _sup_cluster(levels, price)
