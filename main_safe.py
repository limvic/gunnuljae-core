...
async def judge_step1(query: str):
    ...
    # 코스피("J") 먼저 시도 → rt_cd=0이어도 price=0이면 코스닥("Q") 재시도
    # 이유: 코스피 조회 시 코스닥 종목은 껍데기 응답(rt_cd=0, price=0)이 오기 때문
    async with httpx.AsyncClient(timeout=10) as c:
        last_data = None
        for mkt in ("J", "Q"):
            res = await c.get(
            ...
            data = res.json()
            if data.get("rt_cd") == "0":
                price_check = int(data.get("output", {}).get("stck_prpr", 0) or 0)
                if price_check > 0:
                    break          # 실제 데이터 있으면 바로 사용
                last_data = data   # 껍데기면 저장 후 다음 시장 시도
        else:
            # J, Q 둘 다 price=0이면 마지막 응답 사용 (준비중 표시)
            if last_data:
                data = last_data
            else:
