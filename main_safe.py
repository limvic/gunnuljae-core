...
async def fetch_price(code: str, token: str) -> dict:
    ...
    return {
        ...
        # ✅ 정정(2026.06.02): hts_kor_isnm(종목명) 우선. bstp_kor_isnm은 업종명이라 fallback
        "name": o.get("hts_kor_isnm") or o.get("bstp_kor_isnm") or code,
        "sector": o.get("bstp_kor_isnm", ""),   # 업종은 별도 필드로 분리(judge와 일관)
    ...
    while True:
        try:
            ...
            if _scheduler_market_open():
                ...
                await _run_strike_scan()   # 🔱 Strike Alert 상시 감시
...
async def judge_step1(query: str):
    """Judge 1단계: 종목명 → 시세/밸류/52주 회복률 자동수집."""
    code = _judge_resolve(query)
    if not code:
        raise HTTPException(
            status_code=404,
            detail=f"'{query}' 코드 못 찾음 — 6자리 코드 입력 또는 JUDGE_STOCK_MAP에 추가",
        )
    ...
    async with httpx.AsyncClient(timeout=10) as c:
        for mkt in ("J", "Q"):
            ...
            raise HTTPException(status_code=502, detail=f"KIS: {data.get('msg1', '조회 실패')}")
    o = data["output"]

    price = int(o.get("stck_prpr", 0))
    w52l  = _to_int_safe(o.get("w52_lwpr"))   # 52주 최저
    w52h  = _to_int_safe(o.get("w52_hgpr"))   # 52주 최고
    # ── 종목명 추출: hts_kor_isnm 우선. 장외에도 채워짐이 원칙.
    # bstp_kor_isnm = 업종명이므로 name으로 절대 사용 금지.
    # 빈 문자열("")도 falsy → or 체인에서 자동 건너뜀.
    raw_name = (o.get("hts_kor_isnm") or "").strip()
    name_out = raw_name if raw_name else code   # 종목명 없으면 코드 표시(준비중 X)
...
# ══════════════════════════════════════════════════════════════════════════════
# 📡 Trinity Auto Brief v1.1
#    단일 Cron → 시간대 자동 판단 → 아침브리핑 or 장마감 리포트 분기
#    채피 설계 × 써니 구현 × 림빅 결정
#    Cron 설정: */15 * * * 1-5  (평일 15분마다 실행, 내부에서 시간 체크)
# ══════════════════════════════════════════════════════════════════════════════

from datetime import datetime as _brief_dt
import zoneinfo as _zi

_KST = _zi.ZoneInfo("Asia/Seoul")

# ── 발송 중복 방지 (당일 발송 기록) ─────────────────────────────────────────
_brief_sent_log: dict = {}   # {"2026-06-03": {"morning": True, "quad": True}}


def _already_sent(date_str: str, kind: str) -> bool:
    return _brief_sent_log.get(date_str, {}).get(kind, False)


def _mark_sent(date_str: str, kind: str):
    if date_str not in _brief_sent_log:
        _brief_sent_log[date_str] = {}
    _brief_sent_log[date_str][kind] = True
    # 오래된 날짜 정리 (최근 7일만 유지)
    keys = sorted(_brief_sent_log.keys())
    for old in keys[:-7]:
        del _brief_sent_log[old]


@app.post("/brief/morning")
async def brief_morning():
    """🌄 아침 브리핑 — 수동 호출 또는 /brief/auto 에서 분기"""
    if not TG_TOKEN or not TG_CHAT_ID:
        raise HTTPException(status_code=500, detail="TG_TOKEN / TG_CHAT_ID 환경변수 없음")

    now = _brief_dt.now(_KST).strftime("%Y-%m-%d %H:%M")
...
async def brief_quad_report():
    """📊 장마감 Quad-Core 리포트 — 수동 호출 또는 /brief/auto 에서 분기"""
    if not TG_TOKEN or not TG_CHAT_ID:
        raise HTTPException(status_code=500, detail="TG_TOKEN / TG_CHAT_ID 환경변수 없음")
    ...
    try:
        ...
        for code, label in quad_codes:
            try:
                ...
                lines.append(f"  • {label}: {arrow} {sign}{chg:.1f}% ({price_val:,}원)")
            except Exception:
                lines.append(f"  • {label}: 준비 중")
    except Exception:
        lines = [f"  • {label}: 준비 중" for _, label in quad_codes]
...
@app.post("/brief/auto")
async def brief_auto():
    """
    ⏰ Trinity Auto Brief — Railway Cron 전용 엔드포인트
    Cron 설정: 0 23 * * 0-4   → KST 08:00 월~금 (아침 브리핑)
               40 6 * * 1-5   → KST 15:40 월~금 (장마감 리포트)
    하지만 Cron 1개 제한 → */15 * * * 1-5 로 실행 후 내부 시간 분기
    중복 발송 방지: 당일 1회만 발송
    """
    now_kst = _brief_dt.now(_KST)
    date_str = now_kst.strftime("%Y-%m-%d")
    hour = now_kst.hour
    minute = now_kst.minute
    weekday = now_kst.weekday()  # 0=월 ~ 4=금

    # 주말 스킵
    if weekday >= 5:
        return {"status": "skipped", "reason": "weekend"}

    result = {"date": date_str, "time": now_kst.strftime("%H:%M"), "actions": []}

    # ── 아침 브리핑: 08:00~08:29 ────────────────────────────────────────────
    if hour == 8 and minute < 30:
        if _already_sent(date_str, "morning"):
            result["actions"].append({"type": "morning", "status": "already_sent"})
        else:
            await brief_morning()
            _mark_sent(date_str, "morning")
            result["actions"].append({"type": "morning", "status": "sent"})

    # ── 장마감 리포트: 15:30~15:59 ──────────────────────────────────────────
    elif hour == 15 and minute >= 30:
        if _already_sent(date_str, "quad"):
            result["actions"].append({"type": "quad", "status": "already_sent"})
        else:
            await brief_quad_report()
            _mark_sent(date_str, "quad")
            result["actions"].append({"type": "quad", "status": "sent"})

    else:
        result["actions"].append({"type": "none", "status": "out_of_window"})

    return result


# ══════════════════════════════════════════════════════════════════════════════
# 🔱 Trinity Strike Alert v1.0 — 백엔드 상시 감시
#    설계: 채피 🔮  |  구현: 써니 ☀️  |  림빅 결정 ✅
#
#    핵심 원칙:
#    "알람은 화면 기능이 아니라 서버 기능이어야 한다"
#
#    발동 조건 (전부 AND):
#    ① IGNITION 상태 = BREAK  (calc_ignition_score + ignition_status 재사용)
#    ② Block Reset 상태 = RECOVERY → BREAK  (JS 로직 Python 이식)
#    ③ 동일 종목코드
#    ④ 두 조건 10분(600초) 이내 동시 충족
#    ⑤ 당일 동일 종목 1회 미발송
#
#    발송 금지: WATCH / READY / SHOCK / ABSORB / ARMED / FIRE_READY 단독
#    프론트 POST(/strike/alert) = 디버그/테스트 전용
# ══════════════════════════════════════════════════════════════════════════════

import time as _st_time
from datetime import datetime as _st_dt
import zoneinfo as _st_zi
from pydantic import BaseModel as _StBaseModel
from typing import Optional as _StOptional

_ST_KST       = _st_zi.ZoneInfo("Asia/Seoul")
_STRIKE_WIN   = 600   # 10분 (초)

# ── Block Reset 감시 대상 Universe ───────────────────────────────────────────
# IGNITION TOP30과 별도로 Block Reset 조건 감시할 종목 목록
# Quad-Core + 주요 관심 종목 (필요 시 확장)
BLOCK_WATCH_UNIVERSE: list = [
...
# ── Strike 상태 저장소 ────────────────────────────────────────────────────────
# _st_pool[code] = {
#   "ignition_break_at": float|None,   # IGNITION BREAK 감지 시각
#   "block_break_at":    float|None,   # Block Reset BREAK 감지 시각
#   "ign_score":         float,
#   "price":             int,
#   "change_pct":        float,
#   "sent_date":         str|None,     # "2026-06-03" 당일 발송 기록
#   # Block Reset 추적용 이전 상태
#   "br_prev_status":    str|None,     # 직전 Block Reset 상태
#   "br_shock_price":    float|None,   # SHOCK 진입 시 가격
#   "br_avg_vol":        float|None,   # SHOCK 진입 시 평균 거래량
#   "br_prev_low":       float|None,   # SHOCK 직전 저가
# }
_st_pool: dict = {}


def _st_get(code: str) -> dict:
    if code not in _st_pool:
        _st_pool[code] = {
            "ignition_break_at": None,
            "block_break_at":    None,
            "ign_score":         0.0,
            "price":             0,
            "change_pct":        0.0,
    ...
    return _st_pool[code]


def _st_already_sent(code: str) -> bool:
    today = _st_dt.now(_ST_KST).strftime("%Y-%m-%d")
    return _st_get(code).get("sent_date") == today


def _st_mark_sent(code: str):
    today = _st_dt.now(_ST_KST).strftime("%Y-%m-%d")
    _st_get(code)["sent_date"] = today


def _st_in_window(ts_a, ts_b) -> bool:
    if ts_a is None or ts_b is None:
        return False
    return abs(ts_a - ts_b) <= _STRIKE_WIN


def _st_can_fire(code: str) -> bool:
    if _st_already_sent(code):
        return False
    s = _st_get(code)
    return _st_in_window(s["ignition_break_at"], s["block_break_at"])


# ── Block Reset 판단 로직 (JS → Python 이식) ─────────────────────────────────
def _calc_block_reset_status(p: dict, st: dict) -> str:
    """
    Block Reset SHOCK/ABSORB/RECOVERY/BREAK 상태 판단.
    p = fetch_price() 결과
    st = _st_pool[code] 저장된 이전 상태
    반환: 'normal'|'shock'|'absorb'|'recovery'|'break'
    """
    cr  = p.get("change_pct", 0.0)
    vr  = p.get("vol_ratio",  0.0)
    price    = p.get("price", 0)
    ma5_est  = price * 0.995   # MA5 추정 (실시간 MA5 없으므로 보수 추정)
    prev_close = p.get("prev_close", price)
    low        = p.get("low", price)

    # ── SHOCK 진입 감지 ──────────────────────────────────────────────────────
    is_shock = cr <= -5.0 and vr >= 1.5

    if is_shock:
        # SHOCK 첫 진입이면 기준값 기록
        if st.get("br_prev_status") not in ("shock", "absorb", "recovery"):
            st["br_shock_price"] = price
            st["br_avg_vol"]     = p.get("volume", 0) / max(vr, 1)
            st["br_prev_low"]    = low
        return "shock"

    # SHOCK 경험 없으면 normal
    if st.get("br_shock_price") is None:
        return "normal"

    # ── SHOCK 이후 ABSORB/RECOVERY/BREAK 판단 ───────────────────────────────
    avg_vol    = st.get("br_avg_vol", 1)
    prev_low   = st.get("br_prev_low", price * 0.97)
    shock_price= st.get("br_shock_price", price)
    shock_mid  = (shock_price + prev_close) / 2

    is_bullish       = price > prev_close
    hold_low         = low >= prev_low * 0.995
    vol_re_inc       = p.get("volume", 0) > avg_vol * 1.3
    above_ma5        = price > ma5_est
    above_shock_mid  = price > shock_mid
    low_breach       = low < prev_low

    if low_breach and vr >= 0.7:
        return "shock"   # 저가 재이탈 → SHOCK 재진입
    elif above_ma5 and above_shock_mid and is_bullish and vol_re_inc:
        return "break"
    elif is_bullish and vol_re_inc and hold_low:
        return "recovery"
    else:
        return "absorb"
...
# ── 텔레그램 Strike 포맷 ─────────────────────────────────────────────────────
def _format_strike_msg(code: str, name: str, price: int,
                        change_pct: float, ign_score: float, now_str: str) -> str:
    arrow = "▲" if change_pct >= 0 else "▼"
    sign  = "+" if change_pct >= 0 else ""
    return (
        f"🔱 <b>TRINITY STRIKE ALERT</b>\n\n"
        f"종목   <b>{name}</b> ({code})\n"
        f"현재가 ₩{price:,}  {arrow} {sign}{change_pct:.1f}%\n\n"
        f"━━━━━━━━━━━━━━━\n"
        f"⚡ IGNITION    <b>BREAK</b>  score {ign_score:.0f}\n"
        f"🔄 BLOCK RESET <b>RECOVERY → BREAK</b>\n\n"
        f"━━━━━━━━━━━━━━━\n"
        f"⏰ {now_str} KST\n"
        f"판단은 림빅 🔱"
    )
...
async def _send_strike_telegram(code: str, name: str, price: int,
                                 change_pct: float, ign_score: float):
    """Strike Alert 텔레그램 발송"""
    if not TG_TOKEN or not TG_CHAT_ID:
        print(f"[STRIKE] TG 환경변수 없음 — 발송 스킵")
        return
    now_str = _st_dt.now(_ST_KST).strftime("%Y-%m-%d %H:%M")
    msg = _format_strike_msg(code, name, price, change_pct, ign_score, now_str)
    try:
        url = f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage"
        async with httpx.AsyncClient(timeout=10) as c:
            res = await c.post(url, json={
        ...
        print(f"[STRIKE] 🔱 발송 {'✅' if ok else '❌'}: {code} {name}")
    except Exception as e:
        print(f"[STRIKE] ❌ 발송 예외: {e}")


# ── 백엔드 Strike 스캔 (scan_loop에서 호출) ──────────────────────────────────
async def _run_strike_scan():
    """
    BLOCK_WATCH_UNIVERSE 종목 전체 순회:
    ① IGNITION 점수 → BREAK 여부 기록
    ② Block Reset 상태 판단 → BREAK 여부 기록
    ③ AND 조건 충족 시 STRIKE ALERT 발송
    """
    try:
        token = await get_token()
    except Exception as e:
        print(f"[STRIKE-SCAN] 토큰 실패: {e}")
        return

    now_ts = _st_time.time()
    fired  = 0

    for entry in BLOCK_WATCH_UNIVERSE:
        code = entry["code"]
        name = entry["name"]
        ...
        ign_item = {
            ...
            "pullback_pct": p.get("pullback_pct", 0),
            "high_ratio": p.get("high_ratio", 100),
            "is_bullish": p.get("is_bullish", False),
        }
        ign_score  = calc_ignition_score(ign_item)
        ign_status = ignition_status(ign_score)

        if ign_status == "BREAK":
            st["ignition_break_at"] = now_ts
            st["ign_score"]         = ign_score
            st["price"]             = p.get("price", 0)
            st["change_pct"]        = p.get("change_pct", 0.0)
            print(f"[STRIKE-SCAN] ⚡ IGNITION BREAK: {code} {name} score={ign_score:.0f}")

        # ── ② Block Reset BREAK 감지 ─────────────────────────────────────────
        prev_br_status = st.get("br_prev_status")
        br_status      = _calc_block_reset_status(p, st)
        st["br_prev_status"] = br_status

        # RECOVERY → BREAK 전환 감지
        if br_status == "break" and prev_br_status == "recovery":
            st["block_break_at"] = now_ts
            if not st.get("price"): st["price"] = p.get("price", 0)
            print(f"[STRIKE-SCAN] 🔄 BLOCK RESET RECOVERY→BREAK: {code} {name}")

        # ── ③ AND 조건 체크 → 발송 ───────────────────────────────────────────
        if _st_can_fire(code):
            price      = p.get("price", st.get("price", 0))
            change_pct = p.get("change_pct", st.get("change_pct", 0.0))
            ign_score_f= st.get("ign_score", ign_score)

            await _send_strike_telegram(code, name, price, change_pct, ign_score_f)
            _st_mark_sent(code)

            # 상태 초기화
            st["ignition_break_at"] = None
            st["block_break_at"]    = None
            st["br_prev_status"]    = None
            st["br_shock_price"]    = None
            fired += 1

    if fired:
        now_str = _st_dt.now(_ST_KST).strftime("%H:%M:%S")
        print(f"[STRIKE-SCAN] 🔱 {now_str} | STRIKE ALERT {fired}건 발송")


# ── 프론트 연동 엔드포인트 (디버그/테스트 전용) ──────────────────────────────
class _StEventIn(_StBaseModel):
    code:       str
    name:       str
    engine:     str            # "ignition" | "block_reset"
    state:      str            # "BREAK" | "RECOVERY_BREAK"
    score:      _StOptional[float] = None
    price:      _StOptional[int]   = None
    change_pct: _StOptional[float] = None


_ST_BLOCKED = {
    "WATCH", "READY", "SHOCK", "ABSORB", "ARMED", "FIRE_READY",
    "watch", "ready", "shock", "absorb", "armed", "fire_ready",
}


@app.post("/strike/alert")
async def strike_alert_frontend(ev: _StEventIn):
    """
    🔱 프론트 → 백엔드 이벤트 전달 (디버그/테스트 전용)
    실전 감시는 _run_strike_scan() 백그라운드 루프가 담당
    """
    if ev.state in _ST_BLOCKED:
        return {"status": "blocked", "reason": f"{ev.state} 발송 금지 상태"}

    code   = ev.code.strip()
    now_ts = _st_time.time()
    st     = _st_get(code)

    if ev.engine == "ignition" and ev.state.upper() == "BREAK":
        st["ignition_break_at"] = now_ts
        st["ign_score"]  = ev.score or 0
        st["price"]      = ev.price or 0
        st["change_pct"] = ev.change_pct or 0.0

    elif ev.engine == "block_reset" and ev.state.upper() in ("RECOVERY_BREAK", "BREAK"):
        st["block_break_at"] = now_ts
        if ev.price:      st["price"]      = ev.price
        if ev.change_pct: st["change_pct"] = ev.change_pct

    else:
        return {"status": "ignored", "reason": "조건 미해당"}

    if _st_can_fire(code):
        p_val = st.get("price", 0)
        c_val = st.get("change_pct", 0.0)
        s_val = st.get("ign_score", 0)
        await _send_strike_telegram(code, ev.name, p_val, c_val, s_val)
        _st_mark_sent(code)
        st["ignition_break_at"] = None
        st["block_break_at"]    = None
        return {"status": "fired", "code": code, "name": ev.name}

    missing = []
    if st["ignition_break_at"] is None: missing.append("IGNITION BREAK 대기중")
    if st["block_break_at"]    is None: missing.append("Block Reset BREAK 대기중")
    if st["ignition_break_at"] and st["block_break_at"]:
        diff = abs(st["ignition_break_at"] - st["block_break_at"])
        if diff > _STRIKE_WIN:
            missing.append(f"10분 초과 ({diff:.0f}초 차이)")
    return {"status": "pending", "missing": missing}


@app.get("/strike/status")
async def strike_status():
    """🔱 Strike 대기 상태 전체 조회 (디버그용)"""
    today  = _st_dt.now(_ST_KST).strftime("%Y-%m-%d")
    now_ts = _st_time.time()
    rows   = []
    for code, s in _st_pool.items():
        ign_at   = s.get("ignition_break_at")
        block_at = s.get("block_break_at")
        rows.append({
            "code":           code,
            "ignition_break": f"{now_ts-ign_at:.0f}초 전" if ign_at else None,
            "block_break":    f"{now_ts-block_at:.0f}초 전" if block_at else None,
            "br_status":      s.get("br_prev_status"),
            "within_window":  _st_in_window(ign_at, block_at),
            "sent_today":     s.get("sent_date") == today,
        })
    return {"today": today, "universe": len(BLOCK_WATCH_UNIVERSE), "stocks": rows}
