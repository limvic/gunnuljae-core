...
def _sup_rr(price, stop, w52_high, levels):
    """R/R 재정의: 하락여력 = 진짜 지지선(손절선) 기준. 상승여력 = 가까운 저항 → 없으면 52주 고점.
    반환: (rr, target, target_source). 산출 불가 시 (None, None, 사유)."""
    if not stop or stop >= price:
        return None, None, "손절선 없음"
    risk = price - stop
    up_levels = [v for (_n, v) in levels if v and v > price]
    if up_levels:
        target, src = min(up_levels), "가까운 저항"
...
async def support_zone(query: str, mode: str = "balance"):
    ...
    rr, target, target_src = _sup_rr(price, stop, w52h, levels)
