...
async def support_zone(query: str, mode: str = "balance"):
    ...
    stop, risk, stop_basis = _sup_stop(clusters, candles, mode, price)
    swing_high = _sup_swing_high(candles, price)
    rr, target, target_src = _sup_rr(price, stop, swing_high, w52h)

    # ── grade 거부권 ("R/R은 유혹, Grade는 브레이크") ──
    ...
    if tradeable:
        verdict = "진입 후보 ({}급) — Grade 통과".format(grade)
    else:
        verdict = "진입 보류 ({}급) — R/R {} 여도 Grade 브레이크".format(
            grade, rr if rr is not None else "-")

    return {
        "engine": "Support Alert v0.3",
