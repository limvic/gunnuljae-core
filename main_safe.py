...
async def judge_step1(query: str):
    ...
    async with httpx.AsyncClient(timeout=10) as c:
        ...
        for mkt in ("J", "Q"):
            ...
            rtcd = data.get("rt_cd")
            try:
                pchk = int((data.get("output") or {}).get("stck_prpr", 0) or 0)
            except Exception:
                pchk = 0
            tried.append({
                "mkt": mkt, "rt_cd": rtcd, "price": pchk,
