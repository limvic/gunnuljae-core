...
async def judge_step1(query: str):
    ...
    # ===== KOSDAQ_FIX_v5 ===== output raw 통째로 노출 (진단용)
    async with httpx.AsyncClient(timeout=10) as c:
        res = await c.get(
            f"{BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-price",
            params={"fid_cond_mrkt_div_code": "J", "fid_input_iscd": code},
    ...
    return {
        ...
        "_fix_version": "KOSDAQ_FIX_v5",
        "_kis_mode": KIS_MODE,
        "_server": ("MOCK" if "vts" in BASE_URL else "REAL"),
        "_rt_cd": data.get("rt_cd"),
        "_msg1": (data.get("msg1") or "").strip(),
        "_output_keys": (list(o.keys())[:15] if isinstance(o, dict) else str(type(o))),
        "_output_type": str(type(_raw_output).__name__),
