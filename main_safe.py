...
async def judge_step1(query: str):
    ...
    # ===== KOSDAQ_FIX_v2 ===== (배포 확인용 표식)
    # J로 코스닥 종목 조회 시 rt_cd=0이지만 price=0 껍데기가 옴 → Q 재시도
    used_mkt = None
    tried = []
    async with httpx.AsyncClient(timeout=10) as c:
        last_data = None
        for mkt in ("J", "Q"):
            ...
            pchk = 0
            try:
                pchk = int(data.get("output", {}).get("stck_prpr", 0) or 0)
            except Exception:
                pchk = 0
            tried.append({"mkt": mkt, "rt_cd": rtcd, "price": pchk})
            if rtcd == "0" and pchk > 0:
                used_mkt = mkt
                break
            if rtcd == "0":
                last_data = data
    ...
    return {
        ...
        # 디버그: 실제 KIS 응답 + 시장 재시도 추적 (배포 후 확인용)
        "_fix_version": "KOSDAQ_FIX_v2",
        "_used_mkt": used_mkt,
        "_tried": tried,
