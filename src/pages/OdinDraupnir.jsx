...
const MOCK = {
  ...
  // v0.2 교체 지점: 고정 종목이 아니라 Trinity Universe / KIS 스캔 결과로 연결
  top: [
  ...
  risk: {
    ...
    newEntries: 2, maxNewEntries: 3,   // 오늘 남은 신규진입 가능 횟수
  },
  vavlog: [
...
function TopRuneStocksCard() {
  return (
    <Card title="Top Rune Stocks" sub="Trinity 스캔 · 룬 점수 상위"
      right={<span style={{ font: `500 9px ${MONO}`, letterSpacing: "0.1em", color: C.textDim, border: `1px solid ${C.borderSoft}`, borderRadius: 5, padding: "2px 7px" }}>SCAN</span>}>
...
function GungnirSignalCard({ mode }) {
  ...
  return (
    <Card title="Gungnir Signal" sub="궁니르 진입 · 청산 신호" glow={!blocked}
...
function RiskManagementCard() {
  const r = MOCK.risk;
  const canTrade = r.daily > r.dailyLimit && r.positions < r.maxPositions && r.newEntries > 0;
  return (
    <Card title="Risk Management" sub="리스크 관리">
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <RiskRow k="오늘 신규진입 가능" v={`${r.newEntries} / ${r.maxNewEntries}`} tone={r.newEntries > 0 ? C.gold : C.down} />
        </div>
      </div>
